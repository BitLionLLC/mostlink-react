...
const EditSite = () => {
  ...
  useEffect(() => {
    ...
    const delayDebounceFn = setTimeout(() => {
      subdomain &&
        subdomain !== editModalOpenedWith.subdomain &&
        axios
          .get(
  ...
  const updateSite = (e) => {
    ...
    axios
      .put(
        `${process.env.REACT_APP_API_BASE}/api/sites/siteId/${editModalOpenedWith.id}`,
        {
          title,
          subtitle,
          subdomain,
      ...
      .then(() => {
        ...
        toast("Site updated!", { type: "success", theme });
        setTitle("");
        setSubtitle("");
        setSubdomain("");
        fetchSites();
      ...
      {isEditModalOpen ? (
        <div>
          ...
          <Portal container={createSiteModalRef.current}>
            <div
              className={styles.editSiteModal}
              onKeyDown={onEscKey}
              tabIndex="0"
            >
