...
const ResendVerification = () => {
  ...
  const onSubmit = (e) => {
    e.preventDefault();
    email && axios
      .post(`${process.env.REACT_APP_API_BASE}/api/users/resend-verification`, {
        email
  ...
  return (
    <div className={styles.resendVerificationContainer}>
      <div className={styles.resendVerification} style={{backgroundColor: themeObj.landingCardBackground}}>
        <h1>Resend account verification email</h1>
        <form className={styles.resendForm} onSubmit={onSubmit} onKeyDown={onKeyDown}>
          <label htmlFor="email">Email address* (must be verified)</label>
          <TextField type="text" value={email} onChange={e => setEmail(e.target.value)} id="email" variant="filled" size="small" className={styles.textField} />
