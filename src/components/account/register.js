...
const Register = () => {
  ...
  useEffect(() => {
    const delayDebounceFn = setTimeout(() => {
      email &&
        axios
          .get(
            `${process.env.REACT_APP_API_BASE}/api/users/register/check-email/${email}`
          )
          .then(() => setEmailError(""))
  ...
  useEffect(() => {
    const delayDebounceFn = setTimeout(() => {
      username &&
        axios
          .get(
            `${process.env.REACT_APP_API_BASE}/api/users/register/check-username/${username}`
          )
          .then(() => setUsernameError(""))
          .catch(() =>
            setUsernameError(
              "That username is already in use. Please try another one."
            )
          );
    }, 2000);
  ...
  const onSubmit = (e) => {
    ...
    const isFilledOut =
      firstName && lastName && email && username && password && passwordAgain;
    const hasError =
      usernameError || emailError || passwordError || passwordAgainError;
    ...
    if (isFilledOut && !hasError) {
      ...
      if (!firstName) {
        setFirstNameError(REQUIRED_FIELD_ERROR);
      }
      if (!lastName) {
        setLastNameError(REQUIRED_FIELD_ERROR);
      }
      if (!email) {
        setEmailError(REQUIRED_FIELD_ERROR);
      }
      if (!username) {
        setUsernameError(REQUIRED_FIELD_ERROR);
      }
      if (!password) {
        setPasswordError(REQUIRED_FIELD_ERROR);
      }
      if (!passwordAgain) {
        setPasswordAgainError(REQUIRED_FIELD_ERROR);
      }
    }
  };
  ...
  const responseGoogle = (response) => {
    ...
    if (Object.keys(profileObj).length) {
      ...
      axios
        .post(
          `${process.env.REACT_APP_API_BASE}/api/users/register/google`,
          {
            firstName,
            lastName,
            email,
  ...
  return (
    <div className={styles.registerContainer}>
      <div className={styles.register}>
        <h1>Register</h1>
        <h3>
          By registering for an account, you automatically agree to the&nbsp;
          <Link
          ...
          and&nbsp;
          <Link style={{ color: themeObj.accentColor }} to="/privacy-policy">
            Privacy Policy
          </Link>
          .
        </h3>
        ...
        >
          ...
          {emailError && emailError === "This email is already in use." && (
            <div className={styles.errorText}>
              {
                <span>
                  Would you like to <Link to="/account/login">log in</Link>{" "}
                  instead?
                </span>
              }
            </div>
          ...
          <div className={styles.passwordAndEyeIcon}>
            <TextField
              type={isPasswordAgainShowing ? "text" : "password"}
              value={passwordAgain}
            ...
            <FontAwesomeIcon
              ...
              onClick={() => setIsPasswordAgainShowing(!isPasswordAgainShowing)}
              className={styles.eyeIcon}
            />
          </div>
